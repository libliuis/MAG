...
# Network Parameters
n_input = 128 * 128  # data input (img shape: 28*28)
...
with tf.Session() as sess:
    ...
    print(len(list))
    result = []
    for batch_id in range(0, len(list) // 10 + 1):
        ...
        pred_result_test = sess.run(pred_result, feed_dict={
                                    x: batch_xs, keep_prob: 1.})
        print(pred_result_test)
        result.extend(pred_result_test)
    print(result)
    i = 0
    ...
    for image in list:
        id_tag = image.find(".")
        name = image[0:id_tag]
        im = Image.open("./test_resize/" + image)
        out = im.resize((128, 128))
        out.save("./predict_result/" + str(result[i]) + name + ".jpg")
        i += 1
    print("Test Finished!")
